    ...
    stage('push changes') {
      steps {
        withCredentials([string(credentialsId: 'github_jenkins_push_token', variable: 'TOKEN')]) {
          ...
          sh 'git status && echo "${PARAM_ALL_FP}" > pushingfile.txt && git config --global user.name "Jenkins dind" && git config --global user.email false@example.com && git add pushingfile.txt && git commit -a -m "made commit in pipeline" && git push --set-upstream origin master'
        }
      }
    }
  }
