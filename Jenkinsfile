  ...
  stages {
    stage('push changes') {
      steps {
        ...
        sh 'echo "${PARAM_ALL_FP}" > pushingfile.txt && git --global user.name "Jenkins dind" && git --global user.email false@example.com && git add pushingfile.txt && git commit -a -m "made commit in pipeline" && git push'
      }
    }
  }
}
